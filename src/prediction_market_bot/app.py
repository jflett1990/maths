...
from prediction_market_bot.risk.simple import allow_trade, validate_decision
from prediction_market_bot.signals.simple import compute_signal
...
def run_once(config_path: str, adapter: KalshiAdapter | None = None) -> int:
    ...
    require_capability(adapter.capabilities, "supports_public_markets")
    require_capability(adapter.capabilities, "supports_order_books")
    permissions = resolve_signal_permissions(cfg.signals)
    recorder.write("signal_permissions", permissions)
    ...
    for pm in raw_markets:
        ...
        signal = {"market_ticker": pm.market_ticker, **compute_signal(yes, no)}
        ok, reason = allow_trade(signal, cfg.min_expected_edge_bps, cfg.risk.max_spread_for_aggressive_bps)
        ...
        valid, validation_reason = validate_decision(pm.status, decision["size"], decision["price"])
        recorder.write("decision_context", {"market_ticker": pm.market_ticker, "allowed": valid, "reason": validation_reason})
